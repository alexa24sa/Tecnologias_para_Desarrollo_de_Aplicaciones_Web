...
window.actualizarEmpleadoEdit = async function (curp) {
  try {
    const response = await axios.get(
      `acciones/getEmpleado.php?id=${curp}`
    );
    if (response.status === 200) {
      const infoEmpleado = response.data; // Obtener los datos del empleado desde la respuesta

      let tr = document.querySelector(`#empleado_${curp}`);
      let tablaHTML = "";
      tablaHTML += `
          <tr id="empleado_${infoEmpleado.curp}">
            <th class="dt-type-numeric sorting_1" scope="row">${
              infoEmpleado.curp
            }</th>
            <td>${infoEmpleado.numero_empleado}</td>
            <td>${infoEmpleado.nombre}</td>
            <td>${infoEmpleado.primer_apellido}</td>
            <td>${infoEmpleado.segundo_apellido}</td>
